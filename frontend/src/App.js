...
function App() {
  ...
  const printAll = () => {
    ...
    pp.forEach(p => {
      html += `<div class="lbl"><div class="hdr"><div><div style="font-size:17px;font-weight:800">GLOBAL FOOD LINK S.L.</div><div style="font-size:9px;color:#666">Etiqueta de Palet</div></div><div style="text-align:right"><div style="font-size:18px;font-weight:800;color:#2563eb">${p.salesOrderNo}</div><div style="font-size:10px;color:#666">${fmtDate(p.shipmentDate)}</div></div></div><div class="sec"><div class="st">Cliente</div><div class="row"><div class="f"><div class="fl">Nombre</div><div class="fv">${p.customerName}</div></div><div class="f"><div class="fl">N</div><div class="fv">${p.customerNo}</div></div><div class="f"><div class="fl">Pais</div><div class="fv">${p.shipToCountry}</div></div></div></div><div class="sec"><div class="st">Producto</div><div class="row"><div class="f"><div class="fl">Articulo</div><div class="fv big">${p.itemNo}</div></div><div class="f" style="flex:2"><div class="fl">Descripcion</div><div class="fv">${p.itemDescription||""}</div></div></div></div><div class="sec"><div class="st">Palet</div><div class="row"><div class="f"><div class="fl">N Palet</div><div class="fv big">${p.internalPalletNo}</div></div><div class="f"><div class="fl">Lote</div><div class="fv big">${p.lotNo}</div></div><div class="f"><div class="fl">Caducidad</div><div class="fv big">${fmtDate(p.expirationDate)}</div></div></div><div class="row" style="margin-top:8px"><div class="f"><div class="fl">Peso</div><div class="fv big">${p.initQuantity} Kg</div></div><div class="f"><div class="fl">Cajas</div><div class="fv big">${p.boxesPerPallet}</div></div><div class="f"><div class="fl">Kg/Caja</div><div class="fv">${p.kgPerBox}</div></div></div></div><div style="border-top:2px solid #000;padding-top:8px;margin-top:12px"><div class="st" style="text-align:center">Codigos GS1-128</div>${p.gs1Line1?`<div style="text-align:center;margin:8px 0"><img src="${window.location.origin}/api/barcode?data=${encodeURIComponent(p.gs1Line1)}&height=14" style="width:90%;height:75px"/><div style="font-family:monospace;font-size:9px;margin-top:2px">${p.gs1Line1HR}</div></div>`:""} ${p.gs1Line2?`<div style="text-align:center;margin:8px 0"><img src="${window.location.origin}/api/barcode?data=${encodeURIComponent(p.gs1Line2)}&height=14" style="width:90%;height:75px"/><div style="font-family:monospace;font-size:9px;margin-top:2px">${p.gs1Line2HR}</div></div>`:""}<div style="border-top:2px solid #000;padding-top:8px;margin-top:8px;text-align:center"><img src="${window.location.origin}/api/barcode?data=${encodeURIComponent(p.gs1Line3)}&height=16" style="width:90%;height:85px"/><div style="font-family:monospace;font-size:11px;font-weight:bold;margin-top:2px">${p.gs1Line3HR}</div></div></div></div>`;
    });
