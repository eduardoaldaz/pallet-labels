...
function App() {
  ...
  const printLabel = (p) => {
    ...
    w.document.write(`<!DOCTYPE html><html><head><style>
      ...
      <div class="sec"><div class="st">Details</div><div class="row"><div class="f"><div class="fl">SSCC</div><div class="fv" style="font-size:11px">${p.sscc||"--"}</div></div><div class="f"><div class="fl">Batch</div><div class="fv big">${p.lotNo}</div></div><div class="f"><div class="fl">Best Before</div><div class="fv big">${fmtDate(p.expirationDate)}</div></div></div>
      <div class="row" style="margin-top:8px"><div class="f"><div class="fl">Net Weight</div><div class="fv big">${p.initQuantity} Kg</div></div><div class="f"><div class="fl">Boxes</div><div class="fv big">${p.boxesPerPallet}</div></div></div></div>
  ...
  const printAll = () => {
    ...
    pp.forEach(p => {
      html += `<div class="lbl"><div class="hdr"><div><div style="font-size:18px;font-weight:800">GLOBAL FOOD LINK S.L.</div></div><div style="text-align:right"><div style="font-size:16px;font-weight:700">${p.externalDocNo||p.salesOrderNo}</div></div></div><div class="sec"><div class="st">Customer</div><div class="row"><div class="f" style="flex:2"><div class="fl">Name</div><div class="fv">${p.customerName}</div></div><div class="f"><div class="fl">Country</div><div class="fv big">${p.shipToCountry}</div></div></div></div><div class="sec"><div class="st">Item</div><div class="row"><div class="f"><div class="fl">Item Code</div><div class="fv big">${p.itemRefNo||p.itemNo}</div></div><div class="f" style="flex:2"><div class="fl">Description</div><div class="fv">${p.itemDescription||""}</div></div><div class="f"><div class="fl">ETIN/EAN</div><div class="fv" style="font-size:12px">${p.eanCode||"--"}</div></div></div></div><div class="sec"><div class="st">Details</div><div class="row"><div class="f"><div class="fl">SSCC</div><div class="fv" style="font-size:11px">${p.sscc||"--"}</div></div><div class="f"><div class="fl">Batch</div><div class="fv big">${p.lotNo}</div></div><div class="f"><div class="fl">Best Before</div><div class="fv big">${fmtDate(p.expirationDate)}</div></div></div><div class="row" style="margin-top:8px"><div class="f"><div class="fl">Net Weight</div><div class="fv big">${p.initQuantity} Kg</div></div><div class="f"><div class="fl">Boxes</div><div class="fv big">${p.boxesPerPallet}</div></div></div></div><div style="border-top:2px solid #000;padding-top:8px;margin-top:12px"><div class="st" style="text-align:center">GS1-128</div>${p.gs1Line1?`<div style="text-align:center;margin:8px 0"><img src="${window.location.origin}/api/barcode?data=${encodeURIComponent(p.gs1Line1)}&height=14" style="width:90%;height:75px"/><div style="font-family:monospace;font-size:9px;margin-top:2px">${p.gs1Line1HR}</div></div>`:""} ${p.gs1Line2?`<div style="text-align:center;margin:8px 0"><img src="${window.location.origin}/api/barcode?data=${encodeURIComponent(p.gs1Line2)}&height=14" style="width:90%;height:75px"/><div style="font-family:monospace;font-size:9px;margin-top:2px">${p.gs1Line2HR}</div></div>`:""}<div style="border-top:2px solid #000;padding-top:8px;margin-top:8px;text-align:center"><img src="${window.location.origin}/api/barcode?data=${encodeURIComponent(p.gs1Line3)}&height=16" style="width:90%;height:85px"/><div style="font-family:monospace;font-size:11px;font-weight:bold;margin-top:2px">${p.gs1Line3HR}</div></div></div></div>`;
    });
            ...
            <div style={{background:"#fff",borderRadius:6,padding:16,color:"#000",boxShadow:"0 4px 20px rgba(0,0,0,0.4)",maxWidth:460,margin:"0 auto"}}>
              ...
              <div style={{borderBottom:"1px solid #ddd",paddingBottom:6,marginBottom:6}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
                  {[{l:"SSCC",v:preview.sscc||"--",s:9},{l:"Batch",v:preview.lotNo,s:14},{l:"Best Before",v:fmtDate(preview.expirationDate),s:14},{l:"Net Weight",v:`${preview.initQuantity} Kg`,s:14},{l:"Boxes",v:preview.boxesPerPallet,s:14}].map((f,i)=>(
                    <div key={i}><div style={{fontSize:7,color:"#999",textTransform:"uppercase"}}>{f.l}</div><div style={{fontSize:f.s||13,fontWeight:700}}>{f.v}</div></div>
                  ))}
                </div>
              </div>
              <div>
